...
func startInProcess(ctx *sdkserver.Context, clientCtx client.Context, appCreator servertypes.AppCreator) error {
	...
	//解锁账户
	accountName := ctx.Viper.GetString(flagUnlockKey)
	inBuf := bufio.NewReader(os.Stdin)

	keyringBackend := ctx.Viper.GetString(flags.FlagKeyringBackend)
	rootDir := ctx.Viper.GetString(flags.FlagHome)
	...
	armor, err := kr.ExportPrivKeyArmor(accountName, decryptPassword)
	if err != nil {
		return err
	}
	...
	jsonRPCSrv := jsonrpc.NewService(rpc.GetAPIs(clientCtx, rpcapiArr, *ethermintPrivKey))
